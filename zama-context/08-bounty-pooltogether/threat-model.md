...
- A deterministic local two-user test gives Alice TWAB `400`, Bob TWAB `300`, and aggregate `700`. From the same pre-claim snapshot, both claim transactions use identical calldata, consume `676599` gas in the latest run, and emit identical application-level log structures; authorized decryption returns payout `60` for Alice and `0` for Bob.
...
- The pool now rejects a coordinator request recorded before epoch close, bound to another draw, left unbound, or carrying a request block that differs from the provider. These provenance checks pass locally but are not present in the existing Sepolia deployment.
