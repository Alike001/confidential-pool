...
A recurring candidate now adds permissionless rolling epochs, draw-scoped encrypted TWABs, sequential user checkpoints, KMS-proven public aggregate denominators, and a two-step claim that stays below the HCU-depth cap. Compatibility pool `0xa12962cb07D7aaC4421B6101B32caB1Bc9D5FC47` proved the current winner arithmetic across two wallets and two consecutive Sepolia draws: KMS aggregates were exactly `1080000` and `2000000`; wallet A privately decrypted `100000` then `0`; wallet B privately decrypted `0` twice. The earlier `FHE.mulDiv` executor incompatibility is therefore resolved. This pool is not the final release because its coordinator traps adapter overpayment refunds. Tested operator-only refund recovery is now implemented locally. Remaining work is a refund-safe coordinator and replacement pool deployment, final smoke lifecycle and strict audit, source verification, frontend rebinding/QA, write activation, and submission packaging.
