...
The Sepolia `cUSDTMock` target is verified for the bounded prototype. Earlier pool `0xa4f2c74Fe1325e218AC9cEDc176DA7C4e175f3a2` completed the original bounded lifecycle, and hardened fixed-epoch pool `0xF99747C771c09909f6Ad56F43D742c7757ECD9E0` passed its strict end-to-end auditor. Compatibility pool `0xa12962cb07D7aaC4421B6101B32caB1Bc9D5FC47` then proved the current recurring implementation across two wallets and two consecutive live draws, including the Sepolia-compatible encrypted winner calculation, one private `100000` payout, three private zero payouts, and exact KMS aggregates of `1080000` and `2000000`. It is evidence, not the final release: its coordinator retains provider overpayment refunds without a withdrawal path. The coordinator source now has tested operator-only refund recovery; a fresh coordinator and pool deployment are the remaining onchain promotion step.

The production-oriented React frontend is in [`frontend/`](./frontend). It implements live Sepolia reads, injected-wallet onboarding, lazy Zama browser-SDK encryption and owner decryption, encrypted deposits and withdrawals, recurring checkpoints, and two-step private claims. Writes remain intentionally gated, and its deployment configuration remains provisional until the refund-safe replacement pool passes the final smoke lifecycle and source verification.
