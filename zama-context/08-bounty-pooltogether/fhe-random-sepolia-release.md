...
The write-enabled production frontend is live at <https://confidential-pool-savings.vercel.app>. Vercel deployment `dpl_BGiVyPuMx9VK5RhNtXVRySVB4Zhm` completed successfully and the canonical URL returned HTTP `200`. Desktop visual smoke testing passed. A mock EIP-1193 lifecycle test confirmed zero wallet RPC calls before the Connect click, `eth_requestAccounts` only after the click, silent authorized-session restoration through `eth_accounts`, explicit permission revocation through `wallet_revokePermissions`, and a disconnected state that remains disconnected after refresh. Real injected-wallet signing remains a manual browser QA step.
