...
Current phase map: Phase 1 complete; Phases 2–3 have a passing local feasibility slice; Phase 4 is partially proven live; the Chainlink adapter has two fulfilled Sepolia requests. Live evidence proves encrypted deposit/decryption and separately funded encrypted yield, but the pre-draw audit found realistic-scale overflow and post-epoch withdrawal defects in the immutable deployment. Both are fixed with 11 focused tests, and a replacement pool is required. Phase 5's deposit and reserve-funding mechanisms are proven but must be repeated on the corrected deployment; Phases 6–8 have not started. Production architecture remains gated by the live RNG-backed draw/claim/withdrawal, a real yield adapter, full V5 compatibility, and metadata-leakage review.
...
Status: candidate design integrated into the product-shaped slice. Fixed-epoch TWAB accounting, realistic-scale public-denominator winner composition, operator commit/reveal, provider-backed finalization, ERC-7984 callback/refund behavior, and post-epoch withdrawal pass locally. The first live deposit/decryption and encrypted yield funding remain valid integration evidence, but the deployed instance is superseded. Replacement deployment, RNG-backed draw/claim/withdrawal, real yield integration, and full V5 compatibility remain open.
