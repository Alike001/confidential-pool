...
The Sepolia `cUSDTMock` target is read-only verified: chain ID `11155111`, 6 decimals, ERC-7984 support, and a valid wrapper-registry association. The corrected current lifecycle candidate at `0xa4f2c74Fe1325e218AC9cEDc176DA7C4e175f3a2` now holds an encrypted `1,000,000`-unit principal deposit and encrypted `100,000`-unit yield reserve. Its epoch is closed, its exact one-user aggregate TWAB is `687,777`, and it is ready for fresh RNG request `3` before the draw/claim/withdrawal path.
