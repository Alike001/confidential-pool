...
- The live evidence pool has immutable `epochStart`/`epochEnd`. A recurring candidate now passes 11 focused core, adversarial, and HCU checks; its deployment, KMS keeper, draw/claim, and strict-auditor scripts are implemented. It still needs a fresh multi-wallet/two-epoch Sepolia lifecycle before replacing that deployment.
