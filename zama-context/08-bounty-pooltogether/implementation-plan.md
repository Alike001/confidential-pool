...
Frontend prototype discovery and reintegration are complete. The immediate gate is to harden and freeze the recurring candidate ABI documented in [`rolling-epochs-final-abi.md`](./rolling-epochs-final-abi.md): add multi-user/repeated-draw and failure-recovery vectors, measure gas/HCU, decide the yield boundary, and update deployment/operator scripts. Then deploy and verify a fresh Sepolia release before enabling production frontend writes.
...
2. **Freeze the submission contract scope — candidate implemented locally.** The recurring candidate has a fixed cadence, one coordinator-bound RNG request per epoch/draw, draw-scoped encrypted TWABs, permissionless epoch advancement, encrypted withdrawals, and a KMS-proven public denominator. Seven focused tests pass. Freeze follows multi-user/repeated-draw, recovery, and gas/HCU validation.
